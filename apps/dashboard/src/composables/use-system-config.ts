...
} from "@/services/api/example-system-config.api";

function cloneConfig<S extends z.ZodObject<z.ZodRawShape>>(
  value: Readonly<z.input<S>>
): z.input<S> {
...
export function resolveSystemConfigValue<S extends z.ZodObject<z.ZodRawShape>>(
  ...
  try {
    ...
    return result.success
      ? (result.data as z.input<S>)
      : cloneConfig<S>(fallback);
  } catch {
    return cloneConfig<S>(fallback);
  }
}
...
export function useSystemConfig<S extends z.ZodObject<z.ZodRawShape>>({
  ...
  const {
    ...
    isPending: isGetSystemConfigByKeyQueryPending,
  } = useGetSystemConfigByKeyQuery(key);
  const {
    mutate: createSystemConfigMutate,
    isPending: isCreateSystemConfigPending,
  } = useCreateSystemMutation();
  const {
    mutate: updateSystemConfigMutate,
    isPending: isUpdateSystemConfigPending,
  } = useUpdateSystemConfigByKeyMutation(key);
  const isPending = computed(
    () => isCreateSystemConfigPending.value || isUpdateSystemConfigPending.value
  );
  ...
  watch(
    [systemConfigData, isGetSystemConfigByKeyQueryPending],
    ([configData, isGetting]) => {
      if (isGetting) {
        return;
      }
              ...
              toast("System config created with default value.", {
                description: h(
                  ...
                  h(
                    ...
                    { class: "text-white" },
                    JSON.stringify(
                      { description, key, value: configValue },
                      null,
                      2
                    )
                  )
      ...
      const configValue = resolveSystemConfigValue(
